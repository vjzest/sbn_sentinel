...
def verify_payment(request: VerifyPaymentRequest, db: Session = Depends(get_db)):
    ...
    """
    if not client or "mock" in razorpay_key_id:
        invoice = db.query(Invoice).filter(Invoice.razorpay_order_id == request.razorpay_order_id).first()
        if invoice:
            invoice.status = "Paid"
...
async def razorpay_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not hmac.compare_digest(expected_sig, signature) and "mock" not in razorpay_webhook_secret:
