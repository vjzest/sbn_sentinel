...
class SentinelStateTransitionEngine:
    ...
    def permit_transition(cls, entity_type: str, current_state: str, requested_state: str) -> bool:
        ...
        if entity_type not in cls.VALID_TRANSITIONS:
            logger.warning(f"[SSTE] Unmapped entity type '{entity_type}'. Permitting transition to '{requested_state}'.")
            return True
            
        allowed_states = cls.VALID_TRANSITIONS[entity_type].get(current_state, [])
        if requested_state in allowed_states:
            logger.info(f"[SSTE] Permitted {entity_type} transition: {current_state} -> {requested_state}")
            return True
        ...
        logger.error(f"[SSTE] REJECTED {entity_type} transition: {current_state} -> {requested_state} is not permitted.")
        return False
