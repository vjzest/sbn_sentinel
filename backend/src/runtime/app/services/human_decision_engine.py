...
class HumanDecisionEngine(BaseService):
    ...
    def _process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if rec_record.status != RecommendationStatus.ACTIVE:
            return {"status": "ERROR", "message": f"Recommendation is not decision-eligible. Status: {rec_record.status.value}"}
        ...
        if not auth_config:
            ...
            logger.warning(f"[HumanDecisionEngine] UNAUTHORIZED: No authority config for role {actor_role}.")
            return {"status": "ERROR", "message": "NOT AUTHORIZED."}
            
        if decision_type not in auth_config.allowed_decisions:
            logger.warning(f"[HumanDecisionEngine] UNAUTHORIZED: Role {actor_role} cannot make decision {decision_type.value}.")
        ...
        if is_override and not auth_config.can_override:
            logger.warning(f"[HumanDecisionEngine] UNAUTHORIZED: Role {actor_role} attempted override without permission.")
        ...
        if decision_type in auth_config.requires_reason_for and not reason:
            return {"status": "ERROR", "message": f"A reason is required for decision: {decision_type.value}"}
            
        # 5. Idempotency / Concurrency Check (HDA-025, HDA-026)
        # Check if the recommendation already has an active CURRENT decision.
        existing_decisions = [d for d in governance_registry._human_decisions if d.recommendation_id == recommendation_id and d.status == DecisionStatus.RECORDED]
        if existing_decisions:
            ...
            for d in existing_decisions:
                if d.actor_id == actor_id and d.decision_type == decision_type:
                    ...
                    return {"status": "SUCCESS", "decision_id": d.decision_id, "message": "Decision already recorded."}
            
            # If not identical, it's a contradictory / replacement attempt which V1 restricts for simplicity
            return {"status": "ERROR", "message": "A current decision already exists for this recommendation."}
