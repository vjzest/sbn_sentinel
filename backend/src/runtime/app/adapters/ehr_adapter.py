    ...
    def to_canonical(self,
        ...
        if raw_payload.get("simulate_invalid_response"):
            raise InvalidResponseError(
                "Practice Fusion returned malformed JSON or partial payload.")
        ...
        if "appointment_id" in raw_payload or "pf_appt_id" in raw_payload:
            appointment = CanonicalAppointment(
                appointment_id=str(
                    raw_payload.get(
                        "appointment_id", raw_payload.get(
                            "pf_appt_id", "unknown"))), patient_id=str(
                    raw_payload.get(
                        "patient_id", "unknown")), provider_id=str(
                            raw_payload.get(
                                "provider_id", "unknown")), status=str(
                                    raw_payload.get(
                                        "status", raw_payload.get(
                                            "pf_status", "PENDING"))))
