    ...
    return (
        <div className="bg-white/[0.02] border border-white/10 p-6 rounded-2xl shadow-2xl backdrop-blur-md mt-4 relative overflow-hidden group transition-all hover:bg-white/[0.04]">
            ...
            <h3 className="text-base font-semibold text-white mb-6 tracking-wide flex items-center gap-2">
                <div className="w-1.5 h-4 bg-[var(--color-accent)] rounded-full" />
                Lifecycle Audit Timeline
            </h3>
            
            <div className="relative border-s-2 border-white/10 ms-3 space-y-8 pb-2">
                
                {/* 1. Evidence */}
                {bindings.evidence_refs.length > 0 && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-accent)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(59,130,246,0.5)] transition-transform group-hover/item:scale-125"></div>
                        <h4 className="text-sm font-semibold text-white tracking-wide">Evidence Received</h4>
                        <div className="mt-2 space-y-2">
                            {bindings.evidence_refs.map(e => (
                                <div key={e.evidence_id} className="bg-black/20 border border-white/5 p-2 rounded-lg text-xs text-white/80 font-mono shadow-inner hover:bg-black/40 transition-colors break-all">
                ...
                {bindings.rule_evaluations.length > 0 && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-accent)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(168,85,247,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.rule_evaluations.map(r => (
                                <div key={r.evaluation_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-accent)] font-medium break-all">{r.rule_id} <span className="text-white/40">(v{r.rule_version})</span></div>
                ...
                {bindings.recommendations.length > 0 && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-semantic-positive)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(16,185,129,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.recommendations.map(r => (
                                <div key={r.recommendation_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-semantic-positive)] font-medium break-all">{r.mapping_id} <span className="text-white/40">(v{r.mapping_version})</span></div>
                ...
                {bindings.decisions.length > 0 && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-semantic-attention)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(245,158,11,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.decisions.map(d => (
                                <div key={d.decision_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-semantic-attention)] font-medium">{d.decision_type} <span className="text-white/60">by {d.actor_id}</span></div>
                ...
                {bindings.actions.length > 0 && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-accent)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(14,165,233,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.actions.map(a => (
                                <div key={a.action_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-accent)] font-medium break-all">{a.action_type} <span className="text-white/60">- {a.status}</span></div>
                ...
                {bindings.actions.some(a => a.attempts && a.attempts.length > 0) && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-semantic-information)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(6,182,212,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.actions.flatMap(a => (a.attempts || []).map(att => (
                                <div key={att.attempt_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-semantic-information)] font-medium">
                ...
                {bindings.actions.some(a => a.outcome) && (
                    <div className="relative ps-8 group/item">
                        <div className="absolute w-4 h-4 bg-[var(--color-semantic-positive)] rounded-full -start-[9px] top-1 border-4 border-[var(--color-canvas)] shadow-[0_0_10px_rgba(20,184,166,0.5)] transition-transform group-hover/item:scale-125"></div>
                        ...
                        <div className="mt-2 space-y-2">
                            {bindings.actions.map(a => a.outcome ? (
                                <div key={a.outcome.outcome_id} className="bg-black/20 border border-white/5 p-3 rounded-lg text-xs text-white/80 shadow-inner hover:bg-black/40 transition-colors flex flex-col gap-1">
                                    <div className="font-mono text-[var(--color-semantic-positive)] font-medium">
