...
# 1. Main Wrappers (must do this first)
code = code.replace(
    ...
    'bg-gradient-to-br from-[#2E1055] to-[#120524] border border-white/10 rounded-[24px] p-6 shadow-[0_20px_50px_rgba(46,16,85,0.3)] card-hover flex flex-col justify-between min-h-[160px] transition-all duration-300 text-white'
)
code = code.replace(
    'bg-white border border-[#E8EDF5] rounded-[24px] p-8 premium-shadow col-span-2 flex flex-col justify-between',
    'bg-gradient-to-br from-[#2E1055] to-[#120524] border border-white/10 rounded-[24px] p-8 shadow-[0_20px_50px_rgba(46,16,85,0.3)] col-span-2 flex flex-col justify-between text-white'
)
code = code.replace(
    'bg-white border border-[#E8EDF5] rounded-[24px] p-6 premium-shadow flex-1 flex flex-col justify-between',
    'bg-gradient-to-br from-[#2E1055] to-[#120524] border border-white/10 rounded-[24px] p-6 shadow-[0_20px_50px_rgba(46,16,85,0.3)] flex-1 flex flex-col justify-between text-white'
)
...
code = code.replace(
    ...
    'absolute right-0 mt-3 w-48 bg-[#120524] border border-white/10 rounded-[16px] shadow-[0_20px_50px_rgba(46,16,85,0.3)] z-50 text-white'
)
code = code.replace(
...
code = code.replace('bg-[#EEEAFE]/20', 'bg-purple-500/10')
code = code.replace('hover:bg-[#EEEAFE]/50', 'hover:bg-purple-500/20')
...
# Now carefully replace remaining bg-white with bg-transparent using regex word boundaries
...
print("Fixed ScheduleOptimizerView safely v3")
