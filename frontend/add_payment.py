...
# 2. Modify "Edit Card" and Plan onClick
old_edit_card = '''                    <button className="text-xs font-bold text-[#2563EB] hover:underline cursor-pointer">Edit Card</button>'''
...
import re
code = re.sub(
    r'<button\s*onClick=\{[^}]*\}\s*className="text-xs font-bold text-\[\#2563EB\] hover:underline cursor-pointer"\s*>\s*Edit Card\s*</button>',
...
# 3. Add Payment Modal UI before Toast
